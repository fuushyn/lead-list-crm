#!/usr/bin/env python3
"""Barebones outreach CRM — a local web app with two buttons over CSV "lists".

  Button 1  Send invites N   -> sends LinkedIn connection requests to the next N
                               UNINVITED rows of the active list, paced w/ random waits.
  Button 2  Update CRM        -> reconciles invites from LinkedIn's Sent box, then
                               re-checks accepts / messaged / replied and recomputes
                               next_followup (invited-only).
  Button 3  Update all        -> same, but for EVERY contact, not just invited.

Multiple lists = multiple `crm*.csv` files in the working dir, shown as tabs
(crm.csv -> "main", crm_procore.csv -> "procore", ...). Each list is independent.
...
def account_blocked():
    ...
    if st != 200: return f"account check failed (HTTP {st})"
...
def reconcile_sent(rows):
    """Fold in invites present in LinkedIn's Sent box but not marked here (manual sends)."""
    sent = {}; cur = None
    for _ in range(20):
...
def do_update(scope, path):
    rows = load(path)
    STATE["msg"]="Reconciling sent invites…"
    recon = reconcile_sent(rows)
    ...
    for i, r in enumerate(inv, 1):
        ...
        if cid:
            r["messaged"]="yes"; r["replied"]="yes" if has_reply(cid) else "no"
        time.sleep(0.5)
        if r["replied"]=="yes":
            ...
            r["next_followup"]=ds(today())
        elif r["invite_status"]=="accepted" and r["messaged"]=="yes":
            r["next_followup"]=ds(today()+timedelta(days=3))
        else:
            r["next_followup"]=""
        save(path, rows)
...
def page(active):
    ...
    tabs="".join(f'<a class="tab {"on" if lbl==active else ""}" href="/?list={lbl}">{html.escape(lbl)} <small>{sum(1 for _ in open(p))-1}</small></a>' for lbl,p in lists().items())
    h=html.escape
...
.tab.on{{color:#111;background:#111;color:#fff}} .tab small{{opacity:.55;font-weight:500}}
...
<div class=actions>
 <form method=post action=/send style="display:flex;gap:8px;align-items:center;margin:0"><input type=hidden name=list value="{h(active)}">
   <button {'disabled' if STATE['busy'] else ''}>Send invites</button><input name=n type=number value=10 min=1 max=50> next uninvited</form>
 <form method=post action=/update style=margin:0><input type=hidden name=list value="{h(active)}"><button {'disabled' if STATE['busy'] else ''}>Update CRM</button></form>
 <form method=post action=/update_all style=margin:0><input type=hidden name=list value="{h(active)}"><button {'disabled' if STATE['busy'] else ''} title="re-check every contact, not just invited">Update all</button></form>
</div>
<div class=status>{h(STATE['msg'])}</div>
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not STATE["busy"]:
            if p=="/send":   run_bg(do_send, max(1,min(50,int(body.get("n",["10"])[0]))), path)
            elif p=="/update": run_bg(do_update, "invited", path)
            elif p=="/update_all": run_bg(do_update, "all", path)
